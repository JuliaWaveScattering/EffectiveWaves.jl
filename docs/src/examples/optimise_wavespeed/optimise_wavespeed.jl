...
function f_slow1(sps::Array{Specie}, medium::PhysicalMedium,ωs,kTs)
  N = Int(round(length(ωs)/2))
  mean(imag(kTs[1:N])) + mean(ωs[1:N]./real(kTs[1:N]))
end
function f_slow2(sps::Array{Specie}, medium::PhysicalMedium,ωs,kTs)
  N = Int(round(length(ωs)/2))
  mean(imag(kTs[N:end])) + mean(ωs[N:end]./real(kTs[N:end]))
end
function f_fast1(sps::Array{Specie}, medium::PhysicalMedium,ωs,kTs)
  N = Int(round(length(ωs)/2))
  mean(imag(kTs[1:N])) - mean(ωs[1:N]./real(kTs[1:N]))
end
function f_fast2(sps::Array{Specie}, medium::PhysicalMedium,ωs,kTs)
  N = Int(round(length(ωs)/2))
  mean(imag(kTs[N:end])) - mean(ωs[N:end]./real(kTs[N:end]))
end
