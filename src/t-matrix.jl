...
#     f0 = ω^(2.0)/(medium.c^2)
#     basis_order = 0
#     next_order = -4im*sum(number_density(sp) * Zn(ω,sp,medium,basis_order) for sp in species)
#
#     basis_order = 1
#     next_order = -4im*sum(sp.num_density*Zn(ω,sp,medium,basis_order) for sp in species)
#
...
#         t_vecs[i][-n+basis_order+1] = t_vecs[i][n+basis_order+1]
#     end
#     return t_vecs
# end
#
