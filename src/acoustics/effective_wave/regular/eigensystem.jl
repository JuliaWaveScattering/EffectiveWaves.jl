...
function eigensystem(ω::T, medium::Acoustic{T,2}, micro::ParticulateMicrostructure{2}, ::WithoutSymmetry{2};
        basis_order::Int = 2,
        basis_field_order::Int = 2*basis_order,
        numberofparticles::Number = Inf,
        kws...) where T<:AbstractFloat
...
function eigensystem(ω::T, medium::Acoustic{T,2}, micro::ParticulateMicrostructure{2}, ::RadialSymmetry{2};
    kws...) where {T<:AbstractFloat}

        MM = eigensystem(ω, medium, micro, PlanarSymmetry{2}(); kws...)
        return MM
end
