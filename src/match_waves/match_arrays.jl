...
function match_arrays(ω::T, wave_effs::Vector{EffectivePlaneWaveMode{T,2}}, L::Int, X::AbstractVector{T}, source::PlaneSource{T,2,1,Acoustic{T,2}}, material::Material{2,Halfspace{T,2}};
    ...
    w_vec = T(2) .*
        [
            sum(
                ...
                # sum(wave_effs[i].eigenvectors[m+ho+1,s,:])
                wave_effs[i].eigenvectors[m+ho+1,s,1]
        ...
        [
            sum(
                ...
                exp(im*X[J+1] * kcos_effs[p] - im*n*θ_effs[p]) *
                 wave_effs[p].eigenvectors[n+ho+1,s,1] / (kcos_effs[p] + cos_in)
            for n = -ho:ho, s = 1:S)
    ...
    Rs = [
        ...
        (l+q <= J) ?
            zero(Complex{T}) :
            sum(
                (number_density(species[s])/(k^2)) * t_vecs[s][m+ho+1,m+ho+1] * im^T(n) * wave_effs[p].eigenvectors[n+ho+1,s,1] *
