...
"
function match_arrays(ω::T, wave_effs::Vector{EffectiveWave{T}}, XL::Int, X::AbstractVector{T},
    ...
    σ = integration_scheme(X[1:XL]; scheme=scheme) # integration scheme: trapezoidal

    w_vec = T(2) .*
        [
            sum(
                exp(im*m*(θin - w.θ_eff) + im*X[XL]*(w.k_eff*cos(w.θ_eff) - cos(θin))) *
                species[s].num_density * Z[m,s] *  w.ampsitudes[m+ho+1,s]
    ...
    S_mat = OffsetArray{Complex{T}}(1:XJ, -2ho:2ho);
    for j = 1:XJ, m = -2ho:2ho
        S_mat[j,m] = integrate_S(m, X[j]; θin = θin)
    end
    Es = [
        [
            sum(
                species[s].num_density * Z[n,s] * im^T(n+1) * S_mat[XJ-l,n-m] *
                exp(im*X[XJ]*wave_effs[p].k_eff*cos(wave_effs[p].θ_eff) - im*n*wave_effs[p].θ_eff) *
                 wave_effs[p].amplitudes[n+ho+1] / (wave_effs[p].k_eff*cos(wave_effs[p].θ_eff) + cos(θin))
            for n = -ho:ho, s = 1:S)
        for l = 1:XJ, p in eachindex(wave_effs)]
    for m = -ho:ho]

    # q = min(Int(floor(a12k/(X[2]-X[1]))),XJ)
    # B_mat = OffsetArray{Complex{T}}(1:q, -2ho:2ho);
    # for j = 1:q, m = -2ho:2ho
    #     B_mat[j,m] = integrate_B(m, X[j], sqrt(abs(a12k^2 -X[j]^2)); θin = θin)
    # end
    # σ = integration_scheme(XJ:; scheme=scheme) # integration scheme: trapezoidal
    # Bs = [
    #     [
    #         sum(
    #             species[s].num_density * Z[n,s] * im^T(n+1) * S_mat[XJ-l,n-m] *
