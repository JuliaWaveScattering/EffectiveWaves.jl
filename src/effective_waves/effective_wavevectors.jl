...
function effective_wavevectors(ω::T, k_eff::Complex{T}, medium::Acoustic{T,2}, species::Vector{Specie{T}};
        # dim = 2,
        tol::T = 1e-5,
        kws...) where T<:Number
...
function scale_amplitudes_effective(ω::T, wave_eff::EffectiveWave{T},
        medium::Acoustic{T,2}, species::Vector{Specie{T}};
...
function wienerhopf_wavevectors(ω::T, k_eff::Complex{T}, medium::Acoustic{T,2}, species::Vector{Specie{T}}; kws...) where T<:AbstractFloat
...
function wienerhopf_wavevectors(ω::T, k_effs::Vector{Complex{T}}, medium::Acoustic{T,2}, species::Vector{Specie{T}};
    ...
    as = [
        (outer_radius(s1) * s1.exclusion_distance + outer_radius(s2).r * s2.exclusion_distance)
    ...
    function dSQ0_eff(S,j,l)
        ...
        T(2) / (S^T(2) - (k*as[j,l])^T(2)) * (
            S +
            pi * as[j,l]^T(2) * species[l].num_density*t_vecs[l][m+ho+1,m+ho+1] * (
    ...
    function q(s,j,l,m,n)
        ...
        T(2) * pi * as[j,l]^T(2) * species[l].num_density * t_vecs[l][m+ho+1,m+ho+1] *
