...
using Plots; pyplot()
scatter(avg_wave_X.X, real.(avg_wave_X.amplitudes[:,1]))

XJ = Int(round(length(X)/2 + 10))
XL = Int(round(length(X)/2 - 10))
...
decay_effs = abs.(exp.((im*X_match[1]/k).*k_effs_all))
...
invVY = match_only_arrays(ω, wave_effs, XL, X[1:XJ], medium, [specie]; θin = θin)
...
# check matching
    ...
    ps = map(0:ho) do n
        ...
        plot!(avg_wave_eff_match.X, real.(avg_wave_eff_match.amplitudes[:,n+ho+1]), lab = "Wave eff match")
