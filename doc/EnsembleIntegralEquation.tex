...
\end{multline}
\begin{equation}
  \nfrac {} \sum_{n,j \geq 0}  \scatZ_n  \sigma_j  \left ( \ee^{-\ii x_\ell \cos \theta_\inc } S_{n-m}^j
...
\begin{equation}
  P_{n}^{\ell j} =  \ee^{-\ii x_\ell \cos \theta_\inc } \sigma_j S_{n}^j, \quad Q_{n}^{\ell j} = \sigma_j (B_{n}^{j-\ell} - S_{n}^{j -\ell} ) \chi_{\{|j-\ell| \leq p\}},
\end{equation}
